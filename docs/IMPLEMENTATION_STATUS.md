...
1. TypeScript strict compilation: PASS. 2. ESLint: 0 errors. 3. Tests: 67/67 PASS (two runs). 4. Demo twice: byte-identical, event-log hash 3098dcc2c7038e4309f922e5eeea75acfb2efdcdd6658bf79d703f41a39901a1, final CLOSED v23. 5. No DSH import: verified by grep sweep. 6. No network: no http/net/fetch/DNS imports; no URLs in source. 7. No model calls: FakeAgentRuntime only. 8. Normative inputs byte-identical post-build (hashes above). 9. No public-action implementation (PUBLIC_ACTION_* fail closed with PUBLICATION_DISABLED). 10. All sources under colony-kernel/. 11. schemaVersion on every persisted type. 12. All state changes flow through events. 13. No known unresolved P0 in the M1A surface. 14. Deferred work listed here. 15. Independent review PENDING (PENDING_RATE_LIMIT); the review workspace is external to this repository and was untouched by this release. 16. Post-1A review fixes: expiry-at-commit now enforced (EXPIRED_AT_COMMIT + durable rejection); canonical subject sort unified on the domain tuple comparator (event-log hash updated accordingly); dead `purposeMatchesTransition` helper removed.
17. Post-1A refactor + telemetry invariant (2026-09-25, no behavior change to any gate): dead `MissionState` construction removed from `createMission` (two discarded seeds); the approval-request and approval-record shapes extracted to `openApprovalRequest`/`buildApprovalRecord` (one home each); the in-memory creation seed now calls the domain `initialMissionState` factory instead of re-spelling the shape; unused bindings dropped from `run-demo.ts`. **Telemetry:** every accepted command now emits exactly one entry — `requestPackageApproval` and `requestRevision` commit through `storage.atomicApply` directly and were silent, contradicting `docs/ARCHITECTURE.md` ("Control flow of one command" ends in "telemetry record") and v0.1 §15's derived-view rule. New guard K-01 (`tests/unit/telemetry.test.ts`) fails if either path stops emitting; verified to fail with the emission reverted. Demo output is byte-identical to the pre-refactor tree (sha256 262d937b98e9ce4460bef67132054ed7b9c320cb2031c63d5c1779819b02dc73). Test count 66 → 67 here and on the live surfaces (AGENTS.md, README.md, GOVERNANCE.md); `docs/TEST_TRACEABILITY.md` and `reports/**` are **Milestone 1A records** and stay at 66: `reports/milestone-1a-manifest.json` lists the per-file SHA-256 hashes and test tally of the **0.1.0 release tree**, and is deliberately not re-frozen for later work (records are superseded, never rewritten). It is not merely this change that diverges from it — 8 of its 45 paths already disagreed with HEAD before this change, and none of those 8 were touched here: `.gitignore`, `package.json`, `docs/INPUT_MANIFEST.json`, `reports/MILESTONE_1A_VERIFICATION.md`, the manifest's own self-hash, and a `.prettierrc.json` that no longer exists. `README.md` and this file were already stale too. So the manifest is a snapshot, not a live gate (nothing in `scripts/` or CI reads it), and re-freezing it would rewrite a milestone record rather than keep one true.
